...
This report is generated from the **filtered** patient and microbiology datasets in the ACORN App. Using different filter values, you can generate as many reports as required.
...
```{r}
if(length(input$filter_method_collection) == 2)  feedback_text <- paste0("All methods of collection for specimens. ")
if(length(input$filter_method_collection) == 1) {
  ...
  if(input$filter_method_collection == "other_not_blood") feedback_text <- paste0("(Filter) Methods of collection for specimens: ", paste(input$filter_method_other, collapse = ", "))
}
