...
Patients enrolled between `r min(patient()$date_enrollment)` and `r max(patient()$date_enrollment)`.



```{r dpi = 200, fig.width = 10, fig.height = 6, eval = FALSE}
patient_filter() %>%
  # patient %>%
...
test_data <- function(mic = microbio_filter_report(), pat = patient_filter(), organism_input) {
  data <- left_join(
    mic,
...
ggplot_sir <- function(mic = microbio_filter_report(), pat = patient_filter(), corresp = corresp_org_antibio(), organism_input, matching_name_column, title, surv_cat) {
...
```{r fun_table_1000enrol}
table_1000enrol <- function(mic, org) {
  
  # Add surveillance category, surveillance diagnosis to microbio
  microbio2 <- left_join(mic %>%
                           mutate(episode_id = as.character(episode_id)), 
                         patient_filter() %>% select(surveillance_cat, surveillance_diag, episode_id) %>% 
                           mutate(episode_id = as.character(episode_id)),
                         by = 'episode_id')
  ...
  dta <- left_join(
    ...
    ungroup() %>%
    complete(surveillance_cat, surveillance_diag, fill = list(episode = 0, all = 0, per1000 = 0))
...
```{r fun_table_1000blood}
table_1000blood <- function(mic, org) {
  ...
  if(nrow(dta) >= 1) {
    return(
      flextable(dta) %>%
        set_header_labels(surveillance_cat = "", surveillance_diag = "", episode = "Enrolments w/ Blood Culture", all = "Total Enrolments", per1000 = "per 1,000") %>%
        bold(j = ncol(dta)) %>%
        bold(part = "header") %>%
        merge_v(j = ~ surveillance_cat) %>%
        autofit()
...
match <- "a_baumannii"
show <- test_data(mic = microbio_filter_report(), pat = patient_filter(), organism_input = org)
```
...
`r if(show){"*Acinetobacter baumannii* per 1,000 enrolments:"}`

```{r, eval = show}
table_1000enrol(mic = microbio_filter(), org = org)
```

`r if(show){"*Acinetobacter baumannii* per 1,000 blood cultures:"}`

```{r, eval = show}
table_1000blood(mic = microbio_filter_report(), org = org)
```
...
`r if(show){"*Escherichia coli* per 1,000 enrolments:"}`

```{r, eval = show}
table_1000enrol(mic = microbio_filter(), org = org)
```

`r if(show){"*Escherichia coli* per 1,000 blood cultures:"}`
...
`r if(show){"*Klebsiella pneumoniae* per 1,000 enrolments:"}`

```{r, eval = show}
table_1000enrol(mic = microbio_filter(), org = org)
```

`r if(show){"*Klebsiella pneumoniae* per 1,000 blood cultures:"}`
...
`r if(show){"*Staphylococcus aureus* per 1,000 enrolments:"}`

```{r, eval = show}
table_1000enrol(mic = microbio_filter(), org = org)
```

`r if(show){"*Staphylococcus aureus* per 1,000 blood cultures:"}`
...
`r if(show){"*Streptococcus pneumoniae* per 1,000 enrolments:"}`

```{r, eval = show}
table_1000enrol(mic = microbio_filter(), org = org)
```

`r if(show){"*Streptococcus pneumoniae* per 1,000 blood cultures:"}`
...
```{r}
vec <- unique(microbio_filter_report()$organism)
...
`r if(show){"*Salmonella* combined per 1,000 enrolments:"}`

```{r, eval = show}
table_1000enrol(mic = microbio_filter(), org = org)
```

`r if(show){"*Salmonella* combined per 1,000 blood cultures:"}`
